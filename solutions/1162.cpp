...
int n, k, sk;
void iter(long long a[][maxsk], long long b[][maxsk], long long c[][maxsk]){
    long long R[sk][sk];memset(R, 0, sizeof(R));
...
int main(){
    ...
    cin>>k>>n; sk = 1<<k;
    long long A[sk][maxsk], M[sk][maxsk];
    ...
    for(int s=0;s<sk;s++){
        ...
        while(!Q.empty()){
            int f = Q.front();Q.pop();
            for(int i=1;i<k;i++) if((f&(1<<i)) == 0 && (f&(1<<(i-1))) == 0) M[s][f|(1<<i)|1<<(i-1)] = 1, Q.push(f|(1<<i)|(1<<(i-1)));
        }
    }
